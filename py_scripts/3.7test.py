...
EXP_NAME = f"exp_20260307_V2_refine_{STAMP}"  #保存文件夹路径
...
PATH_INDEX = {
    ...
    "note": "Refine around S2_v4_d14 with fixed best inference thresholds.",
}
...
# Cell 3：定义 S2 周边微调 sweep + 固定最佳推理阈值
import json
from pathlib import Path

print("="*80)
print("🧪 Cell 3 | Define S2 refinement sweep")
print("="*80)

# 当前上一轮训练侧最佳
BEST_S2_MODEL_DIR = Path(
    "/gpfs/share/home/2306391536/projects/cell_seg/Cellpose2TrainDataset/trainset_ft_os3/models/cellpose_1772791990.432032"
).resolve()

# 这里如果路径不对，你把它改成 S2_v4_d14 那个真实 model_dir
# 上面这个值只是占位写法，运行前务必确认
print("BEST_S2_MODEL_DIR:", BEST_S2_MODEL_DIR)
if not BEST_S2_MODEL_DIR.exists():
    print("⚠️ 上面的 BEST_S2_MODEL_DIR 需要你改成真实的 S2 模型路径！")

# 固定当前最优推理阈值（来自 threshold_sweep_eval.csv）
...
SWEEP = [
    dict(tag="T0_d14_refit",          diameter=14, learning_rate=1e-4, weight_decay=1e-2, train_batch_size=32, n_epochs=100, augment=True),
    dict(tag="T1_d13",                diameter=13, learning_rate=1e-4, weight_decay=1e-2, train_batch_size=32, n_epochs=100, augment=True),
    dict(tag="T2_d15",                diameter=15, learning_rate=1e-4, weight_decay=1e-2, train_batch_size=32, n_epochs=100, augment=True),
    dict(tag="T3_d14_lowwd",          diameter=14, learning_rate=1e-4, weight_decay=1e-3, train_batch_size=32, n_epochs=100, augment=True),
    dict(tag="T4_d14_trans",          diameter=14, learning_rate=1e-4, weight_decay=1e-2, train_batch_size=32, n_epochs=100, augment=True, transformer=True),
    dict(tag="T5_d14_lowwd_trans",    diameter=14, learning_rate=1e-4, weight_decay=1e-3, train_batch_size=32, n_epochs=100, augment=True, transformer=True),
]
...
def commit_run(base: dict, variant: dict) -> dict:
    ...
    ctx = {
        ...
        # 训练结束后写入
        "status": "NOT_STARTED",
        "model_dir": None,               # 兼容旧字段：保留
        "final_model_path": None,        # 最终训练完成版本
        "best_model_path": None,         # 解析出的 best checkpoint
        "best_epoch_by_test_loss": None, # 注意：0-based epoch index（来自 log）
        "best_test_loss": None,
        "last_epoch_logged": None,       # 同样是 0-based epoch index
        "last_test_loss": None,
        "best_model_found": False,
        "eval_model_path": None,         # 实际用于推理评估的模型
        "eval_model_type": None,         # best_ckpt / final_model_fallback
        "available_checkpoints": [],
        "checkpoint_strategy": {
            "save_every": p.get("save_every", None),
            "save_each": True,  # 我们后面在 Cell 5 里强制开启
            "epoch_index_base": 0,
...
def build_cmd(ctx: dict):
    ...
    cmd = [
        ...
        "--save_each",  # 关键：显式保存阶段 checkpoint
    ]
...
def scan_available_checkpoints(final_model_path: Path):
    """
    假设 Cellpose 保存规则类似：
      final_model_path = /.../models/cellpose_xxx
      intermediate    = /.../models/cellpose_xxx_epoch_0005
    """
    out = []
...
POLL_S = 60
RUN_ONLY_TAGS = None   # 先试跑可写 ["T0_d14_refit"]
...
BEST_S2_MODEL_DIR = Path(
    "/gpfs/share/home/2306391536/projects/cell_seg/Cellpose2TrainDataset/trainset_ft_os3/models/cellpose_1772798689.3251827"
).resolve()

#cell3没用上的上一把最优的路径，不知道后续要不要用，先改了放这，因为cell3没改就开训了
#cell5的评估里会读取这个路径的进行比较
...
def resolve_best_model_path(final_model_path: str, best_epoch: int):
    """
    best_epoch 是 0-based epoch index（来自训练 log）
    Cellpose 常见中间保存命名：<final_model_path>_epoch_XXXX
    例如 best_epoch=60 -> ..._epoch_0060
    """
    if final_model_path is None:
    ...
    # 若 best_epoch 恰好等于最后一个保存版本但未单独命名，则回退 final
...
for r in runs:
    ...
    for m in MET_RE.finditer(text):
        metrics.append({
            "epoch": int(m.group("epoch")),  # 0-based epoch index
    ...
    if metrics:
        ...
        best_epoch = int(dfm.loc[best_idx, "epoch"])   # 0-based
        best_test  = float(dfm.loc[best_idx, "test_loss"])
        last_epoch = int(dfm.iloc[-1]["epoch"])        # 0-based
        last_test  = float(dfm.iloc[-1]["test_loss"])

        # 这里注意：n_epochs=100 时，若训练脚本按 0,10,20,...,90 打印，
        # 则 last_epoch_logged=90 是正常现象，不代表少训了 10 轮。
...
# 从 snapshot 再补一遍 eval_model_path / eval_model_type，避免 summary 里没写全
runs = [json.loads(l) for l in runs_jsonl.read_text(encoding="utf-8").splitlines() if l.strip()]
...
# summary 可能已有部分字段，这里以 df_extra 为准覆盖
