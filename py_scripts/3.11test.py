...
EXP_NAME = f"exp_20260311_ultrasmall_refine_6runs_400ep_es50_{STAMP}"
...
# Cell 3：定义 6 个版本（ultra-small refinement, 400 cap + ES50）
# 跑400轮，过拟合容忍50轮
import json
from pathlib import Path

print("="*80)
print("🧪 Cell 3 | Define 6-run ultra-small refinement sweep")
print("="*80)
...
SWEEP = [
    # 冠军附近
    dict(tag="R00_wd9e3_lr8e5",   weight_decay=9e-3,  learning_rate=8e-5),
    dict(tag="R01_wd1e2_lr8e5",   weight_decay=1e-2,  learning_rate=8e-5),
    dict(tag="R10_wd9e3_lr1e4",   weight_decay=9e-3,  learning_rate=1e-4),
    dict(tag="R11_wd1e2_lr1e4",   weight_decay=1e-2,  learning_rate=1e-4),  # 锚点复刻冠军

    # 低 lr 支线补点
    dict(tag="R20_wd8e3_lr6e5",   weight_decay=8e-3,  learning_rate=6e-5),
    dict(tag="R21_wd8e3_lr8e5",   weight_decay=8e-3,  learning_rate=8e-5),
]
...
# Cell 13：锁定 R20 / R21，并配置 3D 路径
import json
import pandas as pd
from pathlib import Path

print("="*80)
print("🏷️ Cell 13 | Lock R20 / R21 and setup 3D paths")
print("="*80)
...
assert len(df_rank) > 0, "总榜为空。"

TARGET_TAGS = ["R20_wd8e3_lr6e5", "R21_wd8e3_lr8e5"]
df_pick = df_rank[df_rank["tag"].isin(TARGET_TAGS)].copy()

assert len(df_pick) == 2, f"没能在总榜里同时找到 {TARGET_TAGS}，当前找到 {df_pick['tag'].tolist()}"

# 按你想要的顺序固定
df_pick["tag_order"] = df_pick["tag"].map({t:i for i,t in enumerate(TARGET_TAGS)})
df_pick = df_pick.sort_values("tag_order").reset_index(drop=True)
...
THREED_ROOT = (EXP_DIR / "3d_r20_r21").resolve()
THREED_ROOT.mkdir(parents=True, exist_ok=True)

for m in MODELS_3D:
    out_dir = THREED_ROOT / f"{m['tag']}_3d"
...
THREED_CONFIG = {
    ...
    "note": "Run R20 and R21 on 3D full-brain data directly, skip baseline inference."
}

THREED_CONFIG_PATH = THREED_ROOT / "3d_dual_run_config.json"
...
print("="*80)
print("🖼️ Cell 16 | Slice overlay comparison & local crop analysis for R20 vs R21")
print("="*80)
...
mask_dict = {}
for tag, item in THREED_RESULTS.items():
    mask_dict[tag] = tiff.imread(str(item["mask_path"]))
...
# 固定顺序，防止图左右乱跳
TAG_LEFT = "R20_wd8e3_lr6e5"
TAG_RIGHT = "R21_wd8e3_lr8e5"
assert TAG_LEFT in mask_dict and TAG_RIGHT in mask_dict, f"缺少指定 tag，请检查 THREED_RESULTS keys: {list(mask_dict.keys())}"
...
mask_left_z = mask_dict[TAG_LEFT][Z_LAYER]
mask_right_z = mask_dict[TAG_RIGHT][Z_LAYER]
...
axes[0].imshow(img_z_norm, cmap="gray")
axes[0].imshow(np.ma.masked_where(mask_left_z == 0, mask_left_z), cmap="nipy_spectral", alpha=0.5)
axes[0].set_title(f"{TAG_LEFT}\nCells in Z={Z_LAYER}: {len(np.unique(mask_left_z))-1}",
                  fontsize=18, fontweight='bold')
axes[0].axis("off")

axes[1].imshow(img_z_norm, cmap="gray")
axes[1].imshow(np.ma.masked_where(mask_right_z == 0, mask_right_z), cmap="autumn", alpha=0.5)
axes[1].set_title(f"{TAG_RIGHT}\nCells in Z={Z_LAYER}: {len(np.unique(mask_right_z))-1}",
                  fontsize=18, fontweight='bold')
axes[1].axis("off")

plt.suptitle(f"Whole-brain 2D Slice Overlays (Z={Z_LAYER}) | R20 vs R21", fontsize=22, fontweight='bold', y=1.02)
plt.tight_layout(pad=2.5)

overlay_fig = VIS_DIR / f"slice_overlay_z{Z_LAYER}_R20_vs_R21.png"
...
left_crop = mask_left_z[H_START:H_END, W_START:W_END]
right_crop = mask_right_z[H_START:H_END, W_START:W_END]

fig, axes = plt.subplots(1, 2, figsize=(22, 10))

axes[0].imshow(img_crop_norm, cmap="gray")
axes[0].imshow(np.ma.masked_where(left_crop == 0, left_crop), cmap="nipy_spectral", alpha=0.6)
axes[0].set_title(f"{TAG_LEFT} Crop\nCells in crop: {len(np.unique(left_crop))-1}",
                  fontsize=18, fontweight='bold')
axes[0].axis("off")

axes[1].imshow(img_crop_norm, cmap="gray")
axes[1].imshow(np.ma.masked_where(right_crop == 0, right_crop), cmap="autumn", alpha=0.6)
axes[1].set_title(f"{TAG_RIGHT} Crop\nCells in crop: {len(np.unique(right_crop))-1}",
                  fontsize=18, fontweight='bold')
axes[1].axis("off")

plt.suptitle(
    f"Local Region Comparison (Z={Z_LAYER}) | Crop: H[{H_START}:{H_END}], W[{W_START}:{W_END}]",
    fontsize=20, fontweight='bold', y=1.02
)
plt.tight_layout(pad=2.5)

crop_fig = VIS_DIR / f"slice_crop_overlay_z{Z_LAYER}_R20_vs_R21.png"
...
crop_meta = {
    ...
    "left_tag": TAG_LEFT,
    "right_tag": TAG_RIGHT,
    "left_mask_path": THREED_RESULTS[TAG_LEFT]["mask_path"],
    "right_mask_path": THREED_RESULTS[TAG_RIGHT]["mask_path"],
}
